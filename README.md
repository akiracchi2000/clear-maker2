...
### v2.4.2 (2026-03-21)
*   **[改善] 数学プロンプトの調整**:
    *   ベクトル表記の指示（始点・終点を明示した形式 `\overrightarrow{\mathrm{AB}}`）を追加しました。

### v2.4.1 (2026-03-21)
*   **[追加] カテゴリー定義の更新**:
    *   AIによる躓き分析のカテゴリーに「計算の工夫の有無」を追加しました。

### v2.4.0 (2026-03-20)
*   **[追加] 隠し分析データのメタデータ抽出・収集機能**:
    *   AIの添削出力から躓きポイントの自動分析データ（JSON）を裏側で抽出し、生徒の解答データとともにGAS（スプレッドシート）へ自動送信・記録する機能を追加しました。

### v2.3.0 (2026-03-15)
*   **[追加] AI確信度（Confidence Score）表示**:
    *   AIが生成した解答の確信度を0〜100%で算出・表示する機能を追加しました。
    *   ログ収集（スプレッドシート送信）データにも確信度スコアが含まれるようになりました。
*   **[追加] 確信度の表示/非表示設定**:
    *   設定モーダルに「AI確信度を表示する」チェックボックスを追加しました。
    *   デフォルトではOFF（非表示）です。生徒配布時に確信度を見せたくない場合に便利です。
    *   非表示設定でも、GASへのログ送信には確信度が記録されます。
*   **[追加] 低確信度リトライ＆解の一致チェック**:
    *   確信度90%未満の場合、自動で再生成し2つの回答の解を比較する機能を追加しました。
    *   解が一致すれば「再検証済み ✅」バッジ、不一致の場合は警告バナーと回答切替ボタンが表示されます。
    *   設定モーダルでON/OFF可能（デフォルトOFF、APIコスト最大2倍）。
